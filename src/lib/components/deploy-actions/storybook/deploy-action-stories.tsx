...
const exampleComponents = [
  ...
  },
  {
    ...
    renderComponent: (args) => (
      <DeployActionRow {...args} deploy={MapDeploy(transferDeploy)} />
    ),
  },
  {
    templateLabel: 'Cspr.fun deploy action message:',
    renderComponent: (args) => (
      <DeployActionRow {...args} deploy={MapDeploy(csprFunDeploy)} />
    ),
  },
  {
    templateLabel: 'Auction deploy action message:',
    renderComponent: (args) => (
      <DeployActionRow {...args} deploy={MapDeploy(auctionDeploy)} />
    ),
  },
  {
    templateLabel: 'Associated keys deploy action message:',
    renderComponent: (args) => (
      <DeployActionRow {...args} deploy={MapDeploy(associatedKeysDeploy)} />
    ),
  },
  {
    templateLabel: 'Cspr market deploy action message:',
    renderComponent: (args) => (
      <DeployActionRow {...args} deploy={MapDeploy(csprMarketDeploy)} />
    ),
  },
  {
    templateLabel: 'CEP-18 deploy action message:',
    renderComponent: (args) => (
      <DeployActionRow {...args} deploy={MapDeploy(cep18Deploy)} />
    ),
  },
  {
    templateLabel: 'Nft deploy action message:',
    renderComponent: (args) => (
      <DeployActionRow {...args} deploy={MapDeploy(nftDeploy)} />
    ),
  },
];
