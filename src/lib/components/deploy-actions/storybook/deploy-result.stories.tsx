...
const exampleComponents = [
    {
        ...
        renderComponent: (args) =>  <DeployResultRow {...args} deploy={MapDeploy(transferDeploy)} />
    },{
        templateLabel: 'Cspr.fun deploy results message:',
        renderComponent: (args) =>  <DeployResultRow {...args} deploy={MapDeploy(csprFunDeploy)} />
    },
    {
        templateLabel: 'Auction deploy results message:',
        renderComponent: (args) =>  <DeployResultRow {...args} deploy={MapDeploy(auctionDeploy)} />
    },
    {
        templateLabel: 'Associated keys deploy results message:',
        renderComponent: (args) =>  <DeployResultRow {...args} deploy={MapDeploy(associatedKeysDeploy)} />
    },{
        templateLabel: 'Cspr market deploy results message:',
        renderComponent: (args) =>  <DeployResultRow {...args} deploy={MapDeploy(csprMarketDeploy)} />
    },{
        templateLabel: 'CEP-18 deploy results message:',
        renderComponent: (args) =>  <DeployResultRow {...args} deploy={MapDeploy(cep18Deploy)} />
    },{
        templateLabel: 'Nft deploy results message:',
        renderComponent: (args) =>  <DeployResultRow {...args} deploy={MapDeploy(nftDeploy)} />
    },
]

const TemplateDeployDefaultResults: StoryFn<typeof DeployResultRow> = (args) => {
    return (
        <FlexColumn>
