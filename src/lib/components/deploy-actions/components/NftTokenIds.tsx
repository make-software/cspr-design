...
export const NftTokenId = ({ token_id, path }) => {
  const tokenIdContent = (
    <TokenWrapper key={`token-${token_id}`}>
      <BodyText size={3}>
      <Link color={'primaryBlue'} href={path} ariaDescription={'Link to nft token details'}>
  ...
  return token_id.length > MIN_TRUNCATE_HASH_LENGTH ? (
    <Tooltip tooltipContent={token_id} scale={'xs'} lineHeight={'xs'}>{tokenIdContent}</Tooltip>
  ) : (
    tokenIdContent
  );
};
